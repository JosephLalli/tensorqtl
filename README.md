...
`v_a_i = Var_b(a_i^(b))`, `v_t_i = Var_b(t_i^(b))`. These are treated as **known** measurement variances and enter each channel's weighted regression as absolute precisions `w = 1/(v_inf + τ)`, where `τ` is a per-phenotype, per-channel overdispersion term that is NOT applied by default (`--tau_mode zero`, below; pass `estimate` for the floor) and is what keeps the known-variance SE honest when the Gibbs posterior is not the whole error variance. Weighting is implemented via the sqrt-weight transform (`y* = √w · y`, `x* = √w · x`), which turns weighted least squares into ordinary dot products that vectorize across all *cis* variants on the GPU. Under `--se_mode model` the standard error is the known-variance GLS SE (`se = √(1/xx)`), so inflating a channel's inferential variance genuinely widens its SE and down-weights it in the combination, where an estimated-dispersion WLS would absorb that scale and ignore it. That absolute propagation is exactly what the DEFAULT `--se_mode fitted` gives up: it estimates the dispersion, so only the within-gene SHAPE of the Gibbs variances reaches the answer and their absolute scale cancels.
...
| `--tau_mode` | `zero` (default since 2026-09-21): no additive floor, so the weight is `1/v` and, paired with the default `--se_mode fitted`, the error model is `Var(eps_i) = sigma^2 v_i` — mixQTL's Eq 11 form, in which the weights are a SHAPE and their absolute scale cancels. `estimate` adds a per-phenotype, per-channel moment estimator of overdispersion, fitted under the null model on the samples that carry information (`v_inf > 0`); its denominator is the exact `sum_i w_i(1 - h_i)`, which reduces to DerSimonian-Laird's for an intercept-only design, and it is REQUIRED by `--variance_model two_component`/`library_scaled`. WARNING, and note its scope: `zero` with `--se_mode model` (the known-variance SE) is severely anticonservative — up to 107x nominal type-I, and a combined-statistic calibration of 23.3 — because it then asserts the Gibbs variance IS the whole error variance. With `--se_mode fitted` no such assertion is made and the same pairing calibrates at 1.068. The anticonservatism belongs to the PAIRING, not to `zero` |
...
| `--se_mode` | `fitted` (default since 2026-09-21): the estimated-dispersion SE `sigma_hat/sqrt(xx)`, which makes the weights a shape only — rescaling them all leaves beta and the SE unchanged. `model` is the known-variance GLS SE `sqrt(1/xx)`, which propagates the draws' ABSOLUTE scale into the SE but needs `--tau_mode estimate` to stay calibrated. `robust` is the HC1 sandwich. `map_cis` accepts `fitted` and `model` — `fitted` refits a per-channel residual scale at every permutation, as mixQTL's permutation path does — and rejects `robust`, which has no permutation counterpart |
