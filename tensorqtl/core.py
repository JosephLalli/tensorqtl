...
def calculate_interaction_nominal(genotypes_t, phenotypes_t, interaction_t, residualizer=None,
    ...
    if nps == 1:  # single phenotype case
        ...
        # broadcast instead of materializing two [ng,nb,nps] .repeat() tensors (bit-identical)
        b_se_t = torch.sqrt(Xinv[:, torch.eye(nb, dtype=torch.uint8).bool()].unsqueeze(-1) * rss_t.unsqueeze(1) / dof)
